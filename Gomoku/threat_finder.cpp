...
possible_positions threat_finder_package::substract_threats(const possible_positions& a, const possible_positions& b) {
	...
	set_difference(a.begin(), a.end(), b.begin(), b.end(), inserter(output, output.begin()), threat_finder_package::ordinal_comparer);
	return output;
}
...
uint_least8_t threat_finder_package::threat_to_level(dependency_eval_type type) {
	switch (type) {
		...
		case dependency_eval_type::THREE:
			return 2;
		default: return UINT_LEAST8_MAX;
	}	
}
...
void threat_finder::update_positions(const positions_bundle& black_positions, const positions_bundle& white_positions, figure placed_figure) {
	auto actual_indices = indices_.top(); // intentional copy
	auto actual_output_positions = output_positions_.top();
...
void threat_finder::update_positions(coords black_position, const positions_bundle& white_positions, figure placed_figure) {
	auto actual_indices = indices_.top(); // intentional copy
	auto actual_output_positions = output_positions_.top();
...
void threat_finder::update_positions(const positions_bundle& black_positions, coords white_position, figure placed_figure) {
	auto actual_indices = indices_.top(); // intentional copy
	auto actual_output_positions = output_positions_.top();
...
possible_positions threat_finder::find_dependent_threats(const bit_board& board, coords position, figure attack_figure, uint_fast8_t max_threat_level) {
	...
	for (uint_fast8_t dir = 0; dir < 4; ++dir) {
		...
		if (package.type != dependency_eval_type::NOTHING) {
			for(auto&& offset : package.friendly_response) {
				...
				dependency_eval_package new_package = dependency_eval::evaluate(board.get_line<4>(new_position, attack_figure, direction(dir)));
				auto defence_positions = dependency_eval::offsets_to_positions(new_package.enemy_response, new_position, direction(dir));
				auto level = threat_finder_package::threat_to_level(new_package.type);

				output.push_back(threat_finder_package(new_position, move(defence_positions), direction(dir), new_package.type, level));
			}
		}
	}
...
void threat_finder::build() {
	output_positions_.push(array<possible_positions, 2>());
...
void threat_finder::initialize_position(coords pos, figure attack_figure, indices_array& actual_indices, possible_positions_pair& actual_output_positions) {
		...
		if (package.type != dependency_eval_type::NOTHING) {
			for(auto&& offset : package.friendly_response) {
				...
				if (actual_indices[figure_index][dir][new_position.x][new_position.y] == UINT_LEAST8_MAX) {
					auto new_package = dependency_eval::evaluate(board_->get_line<4>(new_position, attack_figure, direction(dir)));
					auto defence_positions = dependency_eval::offsets_to_positions(new_package.enemy_response, new_position, direction(dir));

					actual_output_positions[figure_index].push_back(threat_finder_package(new_position, move(defence_positions), direction(dir), new_package.type));
					actual_indices[figure_index][dir][new_position.x][new_position.y] = uint_least8_t(actual_output_positions[figure_index].size() - 1);
				}
			}
		}
	}
...
void threat_finder::update_by_attack_position(coords pos, indices_array& actual_indices, possible_positions& actual_output_positions, int_fast8_t figure_index, figure placed_figure) {
	...
	for(uint_fast8_t dir = 0; dir < 4; ++dir) {
		...
		if (package.type != dependency_eval_type::NOTHING) {
			for(auto&& offset : package.friendly_response) {
				coords new_position = pos + coords::DIRECTIONS[dir] * offset;
				dependency_eval_package new_package = dependency_eval::evaluate(board_->get_line<4>(new_position, placed_figure, direction(dir)));
				auto new_defence_positions = dependency_eval::offsets_to_positions(new_package.enemy_response, new_position, direction(dir));

				auto level = threat_finder_package::threat_to_level(new_package.type);
				
				if(actual_indices[figure_index][dir][new_position.x][new_position.y] == UINT_LEAST8_MAX) {
					actual_output_positions.push_back(threat_finder_package(new_position, move(new_defence_positions), direction(dir), new_package.type, level));
					actual_indices[figure_index][dir][new_position.x][new_position.y] = uint_least8_t(actual_output_positions.size() - 1);
				}
				else if (level != actual_output_positions[actual_indices[figure_index][dir][new_position.x][new_position.y]].positions.threat_level) {
					actual_output_positions[actual_indices[figure_index][dir][new_position.x][new_position.y]] = threat_finder_package(new_position, move(new_defence_positions), direction(dir), new_package.type, level);
				}
			}
		}
	}
...
void threat_finder::update_by_defence_position(coords pos, indices_array& actual_indices, possible_positions& actual_output_positions, int_fast8_t figure_index, figure placed_figure) {
	delete_position(actual_indices, actual_output_positions, pos, figure_index);
	...
	for (uint_fast8_t dir = 0; dir < 4; ++dir) {
		for (int_fast8_t offset = -4; offset <= 4; ++offset) {
			...
			if (!new_position.is_out_of_board() && actual_indices[figure_index][dir][new_position.x][new_position.y] != UINT_LEAST8_MAX) {
				dependency_eval_package package = dependency_eval::evaluate(board_->get_line<4>(new_position, placed_figure, direction(dir)));
				auto level = threat_finder_package::threat_to_level(package.type);

				if (level == UINT_LEAST8_MAX) {
					delete_position(actual_indices, actual_output_positions, new_position, direction(dir), figure_index);
				}
				else if (level != actual_output_positions[actual_indices[figure_index][dir][new_position.x][new_position.y]].positions.threat_level) {
					auto new_defence_positions = dependency_eval::offsets_to_positions(package.enemy_response, new_position, direction(dir));
					actual_output_positions[actual_indices[figure_index][dir][new_position.x][new_position.y]] = threat_finder_package(new_position, move(new_defence_positions), direction(dir), package.type, level);
				}
			}
		}
	}
...
possible_positions threat_finder::filter_output_positions(const possible_positions& input_list, uint_fast8_t max_level) {
	possible_positions output_list;
	std::copy_if(input_list.begin(), input_list.end(), std::back_inserter(output_list), [=](threat_finder_package p) {return p.positions.threat_level <= max_level; });
	return output_list;
}

void threat_finder::delete_position(indices_array& indexes, possible_positions_pair& output_positions, coords deleted_position) {
	for (uint_fast8_t dir = 0; dir < 4; ++dir) {
...
void threat_finder::delete_position(indices_array& indexes, possible_positions& output_positions, coords deleted_position, uint_fast8_t deleted_index) {
	for (uint_fast8_t dir = 0; dir < 4; ++dir) {
		delete_position(indexes, output_positions, deleted_position, direction(dir), deleted_index);
	}
}

void threat_finder::delete_position(indices_array& indices, possible_positions& output_positions, coords deleted_position, direction deleted_direction, uint_fast8_t deleted_index) {
	if (indices[deleted_index][deleted_direction][deleted_position.x][deleted_position.y] != UINT_LEAST8_MAX) {
		if (indices[deleted_index][deleted_direction][deleted_position.x][deleted_position.y] == output_positions.size() - 1) {
