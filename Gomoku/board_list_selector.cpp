...
void board_list_selector::full_update(const bit_board& board, coords position) {
	...
	// if the move is not amongs the possible moves, add it
	...
	for (int i = 0; i < 16; ++i) { // loop over all offset that are 1 or 2 pieces apart from $position
		...
		auto black_eval = overlap_eval::evaluate(black_lines);
	...
	// if the move is not amongs the possible moves, ignore it (because moves that are farther 
