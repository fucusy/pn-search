...
overlap_eval::table::table() {
	for (uint32_t line = 0; line < SIZE; ++line) { // creates any possible combination of 9 consecutive moves (each move is a 2-bit value)
...
uint16_t overlap_eval::helper_evaluate_line(const array<figure, 9>& line) {
	int friend_in_five; // number of friend moves in a five-movesframe
	int opponent_in_five;  // number of friend moves in a five-moves frame
	uint16_t output = 0; // cummulated output
	...
	// loop over all possible frames of five moves and cummulate the output
	for (int base = 0; base < 5; ++base) { 
		...
		for (int off = 0; off < 5; ++off) { // loop over all moves in a frame
			...
			else if (line[base + off] == OUTSIDE) { ++friend_in_five; ++opponent_in_five; }
		}
